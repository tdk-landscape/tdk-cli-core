...
- On the first request that hits a deferred resource's route, the wake gateway runs `tilt trigger <resource>` (Tilt's own supported CLI, proven to create-and-start a never-touched resource in one call) plus a separate `tilt trigger` for each of its required dependencies (confirmed necessary: Tilt does not cascade a trigger to a `depends_on` dependency on its own — it just waits on it forever), so the whole chain is prioritized immediately instead of waiting for Tilt's normal resource order. For a previously-run-then-stopped resource, Sablier's existing Docker-level start stays the primary (faster) path, with the same trigger calls fired alongside it so Tilt's own state (e.g. a migrator) catches up too.
- Add a scoped `depends_on` (with Compose health/completion conditions) to a deferred resource's own generated compose entry only, so both Sablier's Docker-level start and the wake gateway know which dependencies (database, NATS, a migrator) must also come up — reusing Compose's and Sablier's own native dependency-waiting instead of a bespoke mechanism; every other resource's compose entry is unaffected and keeps today's `# depends_on removed - Tilt manages service startup order` behavior.
...
- **Engine (Starlark):** `engine/topologies/platform/docker/compose/compose.star` gains a scoped `depends_on` block (with Compose conditions) on a `deferStart` resource's own generated entry only, sourced from the same dependency data `engine/lifecycle/orchestrator.star`'s `should_wait_for_dependencies` already computes.
- **Engine (Starlark):** `engine/topologies/platform/docker/compose/traefik_standalone.star` — adds `--providers.file` support (new command flag, mounted dynamic-config directory) and `--provider.docker.honor-restart-policy=true` on the vendored `sablier` command, plus generated static route entries for deferred resources, and a wake gateway (co-located with the existing `sablier` container) that runs `tilt trigger` and proxies the held request through.
...
- **Tests:** `tests/tilt-engine/` snapshot tests for the per-resource `auto_init` resolution, the generated `depends_on` block, and the generated static Traefik route.
