...
- [ ] 2.1 Add one start-command resolver to the engine. It returns `bun <file>` for a `start` script of exactly `bun run <file>` or `bun <file>`, and `bun run start` otherwise. Verify with a new pytest in `tests/tilt-engine/` that generates Dockerfiles for fixture services with these start scripts: `bun run dist/index.js`, `bun dist/index.js`, `bun run dist/index.js --port 4000`, `NODE_ENV=x bun run dist/index.js`, `tsc && bun dist/index.js`, and no `start`. Assert the resulting `CMD` for each.
- [ ] 2.2 Route every Bun start-command default through the resolver: `l3_builder_layers.star` `RUNTIME_CONFIG`, `dockerfile.star`, `golden_docker_generator_v2.star`, `l4_runtime_layers.star` (backend and the Bun-static frontend at line 213), and `infisical_docker.star` `_runtime_setup`. Verify that `grep -rn "bun run start" engine/` matches only the resolver fallback, and that the 2.1 test passes.
- [ ] 2.3 Emit `init: true` for generated backend and frontend runtime services in `compose.star`. Verify with a pytest asserting `init: true` on both service kinds in a generated compose file.
...
- [ ] 3.1 Set `get_docker_healthcheck_config` to `interval 30`, `start_interval 2`, `start_period 30`, `retries 3`, and add `--start-interval` to every golden `HEALTHCHECK`. Verify with a pytest on the generated `golden-layers.Dockerfile` asserting `--interval=30s --start-interval=2s` on each stage.
- [ ] 3.2 Make `compose.star` (lines 141, 264) and `traefik_standalone.star` read the shared healthcheck config instead of hard-coding `10s`. Verify with a pytest asserting that generated compose files contain `interval: 30s` and `start_interval: 2s` and no `interval: 10s` for app services.
...
- [ ] 3.4 Add a `tdk doctor` check for Docker Engine 25+ and Compose 2.20.2+ in `cli/src/commands/doctor.ts`, with vitest cases in `cli/src/commands/__tests__/doctor.test.ts` for an old version (fails, with an upgrade hint) and a current one (passes). Verify with `bun run test -- doctor.test.ts`.
...
- [ ] 4.2 Verify the feature-gated path: a fixture service with `"featuresEnabled": ["hugo"]` gets `apk add --no-cache hugo` in its generated Dockerfile, and a service without the feature does not (pytest).

## 5. Footprint budget and docs (D6)

- [ ] 5.1 Add `scripts/measure-idle-footprint.sh`. It generates a template backend, starts it, waits for `healthy`, samples `docker stats` for 60 s, and exits non-zero if memory is over 64 MiB or average CPU is 2% or more. Verify it passes against the new layers.
- [ ] 5.2 Document the minimum Docker Engine / Compose versions, the `featuresEnabled: ["hugo"]` migration note, and how to run the footprint script in `README.md`. Verify the documented commands run as written.
