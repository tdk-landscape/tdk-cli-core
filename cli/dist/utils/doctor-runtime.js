 ...
 */
export function parsePublishedPortHolders(dockerPsOutput, ports) {
 ...
 */
export function summarizeTiltBuildError(error) {
    ...
    const missingImage = normalized.match(/No such image:\s*([^\s]+)/i)?.[1] ??
...
export function checkPrivateNpmRegistry(exec = execSync, projectRoot = findProjectRoot() ?? process.cwd(), registryUrl = process.env.VERDACCIO_URL ??
    ...
    if (httpOk) {
        return {
            name: "Private npm registry",
            didPass: true,
            message: `Private npm registry reachable at ${registryUrl}${containerRunning ? " (verdaccio container running)" : ""}`,
        };
    }
    if (containerRunning) {
        return {
...
export function probeContainerRuntimeError(resourceName, exec = execSync) {
    try {
        const names = exec("docker ps -a --format '{{.Names}}'", {
            ...
            .map((line) => line.trim())
            .filter(Boolean);
        const match = names.find((name) => name === resourceName) ??
        ...
        const refusedPkg = logs.match(/ConnectionRefused downloading package manifest (@[A-Za-z0-9._/-]+)/i);
        if (refusedPkg?.[1]) {
        ...
        const structured = logs.match(/error:\s+Failed to [^:\n]+[^\n]{0,120}/i)?.[0] ??
...
export function describeTiltFailure(failure, exec = execSync) {
    if (failure.error) {
        return summarizeTiltBuildError(failure.error);
    }
    if (failure.runtimeStatus === "error") {
        return (probeContainerRuntimeError(failure.name, exec) ??
