...
let embeddedTemplatesCache = null;
...
export class TemplateEngine {
    ...
    registerHelpers() {
        Handlebars.registerHelper("starlark", (value) => {
            return new Handlebars.SafeString(this.formatStarlarkValue(value));
        });
        Handlebars.registerHelper("starlarkArray", (value) => {
...
function dedupePhaseEnabledStacks(phases) {
...
function toComposeProjectName(projectName) {
    return projectName.replace(/-/g, "_");
}
export function generateDatabaseManagementCompose(projectConfig) {
 ...
 */
function hasPhaseShape(value, fieldName) {
    ...
    for (const phaseName of ["pre_alpha", "alpha", "beta", "out_of_scope"]) {
        ...
        if (!phase || typeof phase !== "object")
            return false;
        if (!Array.isArray(phase[fieldName]))
            return false;
    }
    return true;
}
function normalizeProjectConfig(value) {
    ...
    if (hasPhaseShape(config.phases, "enabledStacks")) {
        ...
        const legacyStacks = config.stacks;
        phases = {
            pre_alpha: {
    ...
    const optionalInfra = config.optional_infra;
    ...
    return {
        ...
        phases,
        always_enabled_infra: config.always_enabled_infra,
...
async function warnIfSablierUnlicensed(projectRoot) {
    const resources = discoverResourcesFromRoot(projectRoot).filter((r) => r.config?.sablier?.enable === true);
    if (resources.length === 0)
        return;
    const granted = await hasSablierLicense(projectRoot);
    if (granted)
        return;
    console.warn(`⚠️  Sablier (on-demand scaling) is a Premium feature - no valid license key found. ` +
...
export async function generateMasterConfigs(projectRoot) {
    ...
    const verdaccioInPhase = Object.values(projectConfig.phases).some((phase) => phase.enabledStacks.includes("verdaccio"));
    if (projectConfig.optional_infra.verdaccio || verdaccioInPhase) {
        const granted = await hasVerdaccioLicense(projectRoot);
        if (!granted) {
            console.warn("⚠️  Verdaccio is a Premium feature - no valid license key found. It's enabled in " +
...
function findSelfContainedEngineRoot() {
...
export function ensureProjectRuntimeAssets(projectRoot) {
    ...
    const copied = [];
    if (!sourceRoot) {
        console.warn("⚠️  TDK runtime assets not found - generated Dockerfiles may fail. " +
...
async function vendorTdkExtension(projectRoot) {
    ...
    if (!sourceRoot) {
        console.warn("⚠️  TDK extension not found - set TDK_EXTENSION_SOURCE to its checkout path " +
    ...
    try {
        ...
        console.warn(`⚠️  Premium overlay skipped: ${err.message}`);
    }
}
