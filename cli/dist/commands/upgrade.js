...
async function getLatestBinaryRelease() {
    ...
    if (!assetName)
        return null;
    try {
        const raw = execSync(`curl -fsSL https://api.github.com/repos/${BINARY_RELEASE_REPO}/releases/latest`, { encoding: "utf-8", timeout: 10000 });
...
export async function upgradeViaBinary(tdkPath, release) {
    ...
    try {
        ...
        execSync(`curl -fsSL -o ${JSON.stringify(engineTmpPath)} ${JSON.stringify(release.engineDownloadUrl)}`, { stdio: "pipe", timeout: 120000 });
        ...
        execSync(`tar -xzf ${JSON.stringify(engineTmpPath)} -C ${JSON.stringify(engineDir)} --strip-components=1`);
        ...
        console.log(chalk.cyan(`   curl -fsSL ${release.engineDownloadUrl} | tar -xzf - -C ${engineDir} --strip-components=1`));
        return false;
    }
    finally {
        try {
...
export const upgradeCommand = new Command("upgrade")
    ...
    if (installInfo.method === "binary" && installInfo.path) {
        ...
        if (!binaryRelease) {
            showErrorAndExit(`Could not determine latest release for this platform (${process.platform}/${process.arch})`);
        }
