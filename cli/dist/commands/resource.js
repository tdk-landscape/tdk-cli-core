...
export function createServiceJson(name, type, stack, port, extraFeatures = []) {
    ...
    await runCommand(async () => {
        ...
        if (options.ddd) {
            if (resourceType !== "backend" && resourceType !== "worker") {
                ...
                if (!granted) {
                    throw new Error("DDD scaffolding is a Premium feature and requires a license key that grants it. " +
        ...
        const serviceJson = createServiceJson(resourceName, resourceType, stackName, assignedPort, dddEnabled ? ["ddd"] : []);
